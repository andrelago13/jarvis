...
public class DelayedCommand extends Command {
  ...
  public DelayedCommand(JSONObject command) throws JarvisException {
    super(command);
    mTargetTimestamp = Long
        .parseLong(command.getJSONObject(KEY_TARGET_TIMESTAMP).getString("$numberLong"));
  ...
  public String executeString() {
    return "[DelayedAction] Scheduled action for " + mTimeInfo.toString() + " : " + mCommand
        .executeString();
  }
  ...
  public String friendlyExecuteString() {
    return "Schedule action " + mCommand.friendlyExecuteString().toLowerCase() + " in "
        + TimeUtils.friendlyFormat(mTimeInfo) + ", which is on " + TimeUtils
        .friendlyFormat(mTargetTimestamp);
  }
  ...
  public boolean equals(Command c2) {
    ...
    if (mTimeInfo.value != delayedCommand.mTimeInfo.value
        && mTimeInfo.unit != delayedCommand.mTimeInfo.unit) {
      return false;
    }
