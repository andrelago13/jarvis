...
public class PeriodCommand extends Command {
  ...
  public PeriodCommand(JSONObject command) throws JarvisException {
    ...
    mStartTimestamp = Long
        .parseLong(command.getJSONObject(KEY_START_TIMESTAMP).getString("$numberLong"));
    mEndTimestamp = Long
        .parseLong(command.getJSONObject(KEY_END_TIMESTAMP).getString("$numberLong"));
  ...
  public String executeString() {
    return "[PeriodAction] Scheduled period action " + TimeUtils
        .friendlyFormatPeriod(mStartTimestamp, mEndTimestamp)
  ...
  public boolean equals(Command c2) {
    ...
    if (mStartTimestamp != periodCommand.mStartTimestamp
        && mEndTimestamp != periodCommand.mEndTimestamp) {
      return false;
    }
