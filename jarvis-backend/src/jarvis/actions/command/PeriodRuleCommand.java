...
public class PeriodRuleCommand extends Command {
  ...
  public String executeString() {
    return "[PeriodRuleCommand] Scheduled daily rule from " + TimeUtils
        .localTimeToString(mStartTime)
        + " to " + TimeUtils.localTimeToString(mEndTime) + " : " + mCommand.executeString();
  }

  @Override
  public String undoString() {
    return "[PeriodRuleCommand] Cancelled daily rule scheduling from " + TimeUtils
        .localTimeToString(mStartTime)
        + " to " + TimeUtils.localTimeToString(mEndTime) + " : " + mCommand.executeString();
  }
  ...
  public String friendlyExecuteString() {
    return mCommand.friendlyExecuteString() + " everyday from " + TimeUtils
        .localTimeToString(mStartTime)
  ...
  public boolean equals(Command c2) {
    ...
    if (!mStartTime.equals(periodRuleCommand.mStartTime) && !mEndTime
        .equals(periodRuleCommand.mEndTime)) {
      return false;
    }
